...
\- Run the pipeline via `run\_pipeline.R`:



```r

source("run\_pipeline.R")
...
\- The notebook `notebooks/01\_prepare\_dataset.Rmd` contains exploratory analyses.

\- The pipeline merges \*\*three datasets\*\*: greenspace, physical activity, and participant profile dataset.

\- All scripts in `R/` are sourced in `run\_pipeline.R`.
